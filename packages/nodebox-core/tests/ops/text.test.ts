...
describe('Text Operations', () => {
  ...
  // Full text tests require loading a .ttf font file.

  it('returns empty path when font not loaded', async () => {
    const p = await textpath('hello', 'NonExistentFont', 24, 'left', { x: 0, y: 0 }, 0);
    expect(p.contours.length).toBe(0);
  });

  it('textOnPath returns empty when font not loaded', async () => {
    const path = { contours: [], fill: null, stroke: null, strokeWidth: 1 };
    const p = await textOnPath('hello', path, 'NonExistentFont', 24, 'left', 0, 0);
    expect(p.contours.length).toBe(0);
  });
