//! A single open document and its window: state, panels, and UI loop.
...
/// A request from a document to the application shell.
///
/// Documents cannot manage app-level state (the document list, recent files,
/// quitting), so they push requests that `NodeBoxApp` drains every frame.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AppRequest {
    /// Create a new empty document window.
    NewDocument,
    /// Open the given .ndbx file (in a new window, or a pristine one).
    OpenDocument(PathBuf),
    /// Record a file in the recent-files list.
    AddRecentFile(PathBuf),
    /// Clear the recent-files list.
    ClearRecentFiles,
    /// This document confirmed it can close; remove it from the app.
    CloseDocument,
    /// Quit the application (prompting per dirty document).
    Quit,
}

/// Per-frame context the application shell passes to a document.
pub struct DocumentEnv {
    /// Whether this document is hosted in the root viewport.
    pub is_root: bool,
    /// Whether other documents are open besides this one.
    pub other_documents_exist: bool,
    /// Snapshot of the recent-files list (for the in-window menu).
    pub recent_files: Vec<PathBuf>,
}

/// A single open document with its window state and UI panels.
pub struct Document {
    /// Identifier for this document's viewport (stable across frames).
    pub id: u64,
    /// Requests for the application shell, drained every frame.
    pub requests: Vec<AppRequest>,
    /// Whether this document's window was focused last frame.
    pub focused: bool,
...
impl Document {
    ...
    pub fn new(id: u64, port: Arc<dyn Platform>) -> Self {
    ...
    /// Create a document loaded from `path`.
    ///
    /// On load failure the document stays empty and the error is logged;
    /// a successful load pushes an `AddRecentFile` request.
    pub fn open(id: u64, port: Arc<dyn Platform>, path: &std::path::Path) -> Self {
        let mut doc = Self::new(id, port);
        doc.load_path(path);
        doc
    }

    /// Whether this document is untitled and unmodified, so it can be
    /// replaced in place when opening a file.
    pub fn is_pristine(&self) -> bool {
        self.state.current_file.is_none() && !self.state.dirty && !self.history.can_undo()
    }
    ...
    /// Request opening any .ndbx project files dropped onto the window.
    fn handle_dropped_files(&mut self, ctx: &egui::Context) {
        ...
        for path in dropped {
            self.requests.push(AppRequest::OpenDocument(path));
        }
    }

    /// Intercept window close: prompt to save when there are unsaved changes,
    /// then ask the shell to remove this document.
    fn handle_close_request(&mut self, ctx: &egui::Context, env: &DocumentEnv) {
        if !ctx.input(|i| i.viewport().close_requested()) {
            return;
        }
        if self.state.dirty && !self.allowed_to_close {
            ctx.send_viewport_cmd(egui::ViewportCommand::CancelClose);
            if !self.confirm_discard_changes() {
                return; // User cancelled; keep the window.
            }
            self.allowed_to_close = true;
        }
        // Safe to close. The root viewport is the host OS window for the
        // whole app: when other documents remain, keep it alive and let the
        // shell promote another document into it.
        if env.is_root && env.other_documents_exist {
            ctx.send_viewport_cmd(egui::ViewportCommand::CancelClose);
        } else if env.is_root && self.allowed_to_close {
            // The dirty path sent CancelClose above; re-send the close.
            ctx.send_viewport_cmd(egui::ViewportCommand::Close);
        }
        self.requests.push(AppRequest::CloseDocument);
    }

    /// Handle a menu action routed to this document.
    ///
    /// App-level actions (new/open/quit) become requests for the shell;
    /// document-level actions are applied directly.
    pub fn handle_menu_action(&mut self, action: MenuAction, ctx: &egui::Context) {
        match action {
            MenuAction::New => self.requests.push(AppRequest::NewDocument),
            MenuAction::Open => self.open_file(),
            MenuAction::OpenRecent(path) => self.requests.push(AppRequest::OpenDocument(path)),
            MenuAction::ClearRecent => self.requests.push(AppRequest::ClearRecentFiles),
            ...
            // Quit is app-level: every dirty document gets a prompt.
            MenuAction::Quit => self.requests.push(AppRequest::Quit),
        }
        ctx.request_repaint();
    }

    /// Show the in-window menu bar (non-macOS platforms).
    #[cfg(not(target_os = "macos"))]
    fn show_menu_bar(&mut self, ui: &mut egui::Ui, env: &DocumentEnv) {
        let recent_files_list = &env.recent_files;

        egui::MenuBar::new().ui(ui, |ui| {
            ui.menu_button("File", |ui| {
                if ui.button("New").clicked() {
                    self.requests.push(AppRequest::NewDocument);
                    ui.close();
                }
                ...
                ui.menu_button("Open Recent", |ui| {
                    ...
                    if ui.add_enabled(!recent_files_list.is_empty(), egui::Button::new("Clear Recent")).clicked() {
                        self.requests.push(AppRequest::ClearRecentFiles);
                        ui.close();
                    }
                });
...
impl Document {
    /// Render this document's full window UI into the current viewport.
    ///
    /// Called by the application shell: once per frame for the root
    /// viewport's document and once inside each immediate viewport for the
    /// other documents. `frame` provides the shared wgpu render state.
    #[allow(unused_variables)] // `frame` is unused without gpu-rendering
    pub fn show(&mut self, ctx: &egui::Context, env: &DocumentEnv, frame: &mut eframe::Frame) {
        // Track whether this document's window is focused, so the shell can
        // route menu actions to it.
        self.focused = ctx.input(|i| i.viewport().focused.unwrap_or(false));

        // Keep the window title in sync with the document name and dirty state
        self.update_window_title(ctx);

        // Request opening of .ndbx files dropped onto the window
        self.handle_dropped_files(ctx);

        // Intercept window close to prompt for unsaved changes
        self.handle_close_request(ctx, env);
...
impl Document {
    ...
    /// Path of the file backing this document, if saved.
    ...
    /// The OS window title for this document.
    ...
    pub fn confirm_discard_changes(&mut self) -> bool {
        if !self.state.dirty {
            return true;
        }
    ...
    /// Show the open dialog; the chosen file becomes an open request for
    /// the shell (which decides between a new window and a pristine one).
    fn open_file(&mut self) {
        use nodebox_core::platform::FileFilter;

        match self.port.show_open_project_dialog(&[FileFilter::nodebox()]) {
            Ok(Some(path)) => self.requests.push(AppRequest::OpenDocument(path)),
            Ok(None) => {} // User cancelled
            Err(e) => log::error!("Failed to open file dialog: {}", e),
        }
    }

    /// Load `path` into this document, replacing its content.
    /// Updates the project context and requests a recent-files entry.
    pub fn load_path(&mut self, path: &std::path::Path) {
        ...
        self.requests.push(AppRequest::AddRecentFile(path.to_path_buf()));
    }
        ...
        match self.port.show_save_project_dialog(&[FileFilter::nodebox()], Some("untitled.ndbx")) {
            Ok(Some(path)) => {
                if let Err(e) = self.state.save_file(&path) {
                    ...
                    self.requests.push(AppRequest::AddRecentFile(path));
                }
            }
            Ok(None) => {} // User cancelled
...
mod tests {
    ...
    fn test_handle_point_change_triggers_render() {
        let mut app = Document::new_for_testing();
    ...
    fn test_parameter_change_sets_render_pending_via_check_for_changes() {
        ...
        app.previous_library_hash = Document::hash_library(&app.state.library);
        ...
        let new_hash = Document::hash_library(&app.state.library);
        assert_eq!(
    ...
    fn test_single_change_undo_actually_restores_previous_state() {
        let mut app = Document::new_for_testing_empty();
