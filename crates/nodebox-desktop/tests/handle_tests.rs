...
use nodebox_desktop::{Node, Point, Port};

// Import handle functions directly from the module
use nodebox_desktop::handles::{ellipse_handles, rect_handles, rect_four_point_handle};
...
#[allow(dead_code)]
fn create_ellipse_handles_current_behavior(node: &Node) -> Vec<nodebox_desktop::handles::Handle> {
...
fn create_ellipse_handles_correct_behavior(node: &Node) -> Vec<nodebox_desktop::handles::Handle> {
...
fn test_primitives_handles_read_correct_positions() {
    ...
    nodebox_desktop::populate_default_ports(&mut library.root);
