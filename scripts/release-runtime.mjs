...
function bumpVersion() {
  const packageJson = JSON.parse(readFileSync(RUNTIME_PACKAGE_PATH, "utf8"));
  const currentVersion = packageJson.version;
  const [major, minor, patch] = currentVersion.split(".").map(Number);
  const newVersion = `${major}.${minor}.${patch + 1}`;
  packageJson.version = newVersion;
  writeFileSync(RUNTIME_PACKAGE_PATH, JSON.stringify(packageJson, null, 2) + "\n");
  return newVersion;
}
...
function commitChanges(version) {
  try {
    execSync("git add package-lock.json packages/runtime/package.json");
    execSync(`git commit -m "runtime version ${version}"`);
    execSync(`git tag runtime@${version}`);
  } catch (error) {
...
console.log("Starting @ndbx/runtime release process...");
