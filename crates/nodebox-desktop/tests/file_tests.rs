...
fn test_old_version_files_load_with_warning() {
    ...
    let result = nodebox_core::ndbx::parse_file_with_warnings(&path);
...
fn test_library_files_can_be_loaded() {
    ...
    let library = nodebox_core::ndbx::parse_file(&path).expect("Library files should load");
...
fn test_save_and_reload() {
    ...
    // Serialize to string
    let xml = nodebox_core::ndbx::serialize(&original);

    // Parse back
    let reloaded = nodebox_core::ndbx::parse(&xml).expect("Should be able to parse serialized content");
            ...
            if path.is_dir() {
                ...
                if let Ok(files) = std::fs::read_dir(&path) {
                    for file in files.flatten() {
                        ...
                        {
                            match nodebox_core::ndbx::parse_file(file.path()) {
                                Ok(library) => {
                                    // Basic sanity check
